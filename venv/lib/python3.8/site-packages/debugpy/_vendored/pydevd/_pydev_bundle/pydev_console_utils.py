...
class BaseStdIn:
    ...
    def __getattr__(self, item):
        ...
        raise AttributeError(f"{self.original_stdin} has no attribute {item}")
...
class StdIn(BaseStdIn):
    ...
    def readline(self, *args, **kwargs):
        ...
        try:
            server = xmlrpclib.Server(f'http://{self.host}:{self.client_port}')
...
class BaseInterpreterInterface:
    ...
    def getDescription(self, text):
        try:
            ...
            return '' if obj is None else get_description(obj)
        except:
            return ''
    ...
    def get_server(self):
        if getattr(self, 'host', None) is not None:
            return xmlrpclib.Server(f'http://{self.host}:{self.client_port}')
        else:
            return None
    ...
    def finish_exec(self, more):
        ...
        return server.NotifyFinished(more) if server is not None else True
    ...
    def changeVariable(self, attr, value):
        ...
        def do_change_variable():
            Exec(f'{attr}={value}', self.get_namespace(), self.get_namespace())
