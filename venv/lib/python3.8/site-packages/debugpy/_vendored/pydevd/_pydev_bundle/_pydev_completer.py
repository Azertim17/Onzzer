...
class Completer:
    ...
    def complete(self, text):
        ...
        return self.attr_matches(text) if "." in text else self.global_matches(text)
...
def generate_completions(frame, act_tok):
    ...
    if pydevconsole.IPYTHON:
        return pydevconsole.get_completions(
...
def completions_to_xml(completions):
    ...
    for comp in completions:
        msg.extend(
            (
                '<comp p0="',
                valid_xml(quote(comp[0], '/>_= \t')),
                '" p1="',
                valid_xml(quote(comp[1], '/>_= \t')),
                '" p2="',
                valid_xml(quote(comp[2], '/>_= \t')),
                '" p3="',
                valid_xml(quote(comp[3], '/>_= \t')),
                '"/>',
            )
        )
    msg.append("</xml>")
...
identifier_start = f'{ascii_letters}_'
identifier_part = f'{ascii_letters}_{digits}'
