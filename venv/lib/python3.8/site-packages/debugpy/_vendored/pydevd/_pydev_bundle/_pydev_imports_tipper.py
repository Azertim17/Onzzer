...
def _imp(name, log=None):
    try:
        ...
        if '.' in name:
            ...
            s = f'Unable to import module: {str(name)} - sys.path: {sys.path}'
            if log is not None:
                log.add_content(s)
                log.add_exception()
...
def generate_imports_tip_for_module(obj_to_complete, dir_comps=None, getattr=getattr, filter=lambda name:True):
    ...
    get_complete_info = len(dir_comps) <= 1000
        ...
        try:
            ...
            if get_complete_info:
                try:
                    ...
                    getDoc = not any(isinstance(obj, class_) for class_ in dontGetDocsOn)
                    doc = ''
                    if getDoc:
                        ...
                        try:
                            ...
                            for kwonly_arg in kwonly_args:
                                ...
                                if default is not _SENTINEL:
                                    args.append(f'{kwonly_arg}={default}')
                                else:
        ...
        if len(doc) > 0:
            ...
            if IS_IPY:
                if args.startswith('(self:'):
                    i = args.find(',')
                    args = f'(self{args[i:]}' if i >= 0 else '(self)'
                i = args.find(')')
                if i > 0:
                    args = args[:i + 1]
