    ...
    try:
        ...
        if hasattr(node, 'end_lineno') and hasattr(node, 'end_col_offset'):
            ...
            if offset >= 0 and node.names:
                ...
                offset = -1 if i < 0 else i + len(from_future_import_name)
        if offset >= 0:
...
def _is_managed_arg(arg):
    ...
    return bool(arg.endswith(pydevd_py))
...
def _get_str_type_compatible(s, args):
    ...
    if isinstance(args, (list, tuple)):
        ...
        if type(s) == type(args):
            ...
            return args.encode('utf-8') if isinstance(s, bytes) else args.decode('utf-8')
...
def is_python(path):
    ...
    return any(
        ...
        for name in _get_str_type_compatible(
            filename, ['python', 'jython', 'pypy']
        )
    )
...
def remove_quotes_from_args(args):
    ...
    for x in args:
        ...
        elif Path is not None and isinstance(x, Path):
            x = x.as_posix()
        elif not isinstance(x, (bytes, str)):
            raise InvalidTypeInArgsException(str(type(x)))
        new_args.append(x)
...
def quote_args(args):
    return list(map(quote_arg_win32, args)) if sys.platform == "win32" else args
...
def patch_args(args, is_exec=False):
    ...
    try:
        ...
        params_with_combinable_arg = {'W', 'X', 'Q', 'c', 'm'}
...
def monkey_patch_module(module, funcname, create_func):
    if hasattr(module, funcname):
        original_name = f'original_{funcname}'
...
class _NewThreadStartupWithTrace:
    ...
    def __call__(self):
        ...
        try:
            ...
            if thread_id is not None and py_db is not None:
