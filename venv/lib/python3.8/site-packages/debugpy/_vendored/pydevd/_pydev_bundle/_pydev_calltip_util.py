...
def get_bound_class_name(obj):
    ...
    return None if my_self is None else get_class_name(my_self)
...
def create_method_stub(fn_name, fn_class, argspec, doc_string):
    if fn_name and argspec:
        ...
        if fn_class:
            expr = fn_class if fn_name == '__init__' else f'{fn_class}().{fn_name}'
...
def get_docstring(obj):
    ...
    try:
        if IS_JYTHON:
            ...
            if is_method:
                return ''.join(info.get_as_doc() for info in infos)
        else:
