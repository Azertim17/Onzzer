...
def execute_tests_in_parallel(tests, jobs, split, verbosity, coverage_files, coverage_include):
    ...
    if split == 'module':
        ...
        queue_elements.extend(iter(module_to_tests.values()))
        ...
        for test in tests:
            lst = []
            flatten_test_suite(test, lst)
            queue_elements.extend([test] for test in lst)
        if len(queue_elements) < jobs:
            # Don't create jobs we will never use.
            jobs = len(queue_elements)

    else:
        raise AssertionError(f'Do not know how to handle: {split}')
    ...
    for test_cases in queue_elements:
        ...
        for test_case in test_cases:
            try:
                ...
                test_name = f"{test_case.__class__.__name__}.{test_case._TestCase__testMethodName}"

            test_queue_elements.append(f'{test_case.__pydev_pyfile__}|{test_name}')
    ...
    def run(self):
        try:
            ...
            if self.coverage_output_file and self.coverage_include:
                args.extend((self.coverage_output_file, self.coverage_include))
            import subprocess
