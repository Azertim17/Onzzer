...
def SetTrace(tracing_func):
    ...
    if (
        tracing_func is not None
        and set_trace_to_threads(
            tracing_func,
...
def get_python_helper_lib_filename():
    ...
    if arch.lower() not in ('amd64', 'x86', 'x86_64', 'i386', 'x86'):
        ...
        expected_name = f'attach_{arch}{extension}'
        expected_name_linux = f'attach_linux_{arch}{extension}'
        ...
        suffix = suffix_64 if IS_64BIT_PROCESS else suffix_32
...
def set_trace_to_threads(tracing_func, thread_idents=None, create_dummy_thread=True):
    ...
    for thread_ident in thread_idents:
        ...
        if create_dummy_thread and thread_ident not in threading._active:
