...
if sys.platform == 'win32':
    try:
            ...
            while True:  # Note: while True to make iterative and not recursive
                ...
                if resolved_joined is None:
                    ...
                    for filename in dir_contents:
                        ...
                        raise FileNotFoundError(f'Unable to find: {resolve_lowercase} in {resolved}')
        ...
        def _get_path_with_real_case(filename):
            ...
            try:
                return drive if parts == [''] else _resolve_listing(drive, iter(parts))
...
def set_ide_os(os):
    ...
    if DEBUG_CLIENT_SERVER_TRANSLATION:
        print(f'pydev debugger: client OS: {os}')
    ...
    try:
        ...
        if filename.__class__ != str:
            raise AssertionError(
                f'Paths passed to _abs_and_canonical_path must be str. Found: {filename} ({type(filename)})'
            )
...
def _apply_func_and_normalize_case(filename, func, isabs, normalize_case, os_path_exists=os_path_exists, join=join):
    ...
    if not isabs and not os_path_exists(r):
...
try:
    report = pydev_log.debug if DISABLE_FILE_VALIDATION else pydev_log.critical
    try:
...
def setup_client_server_paths(paths):
    ...
    def _map_file_to_client(filename, cache=norm_filename_to_client_container):
        ...
        try:
            ...
            if (
                translated_normalized.lower() != translated_proper_case.lower()
                and DEBUG_CLIENT_SERVER_TRANSLATION
            ):
                pydev_log.critical(
