...
class ExceptionBreakpoint(object):
    ...
    def __init__(
        ...
        self.qname = qname
        self.name = exctype.__name__ if exctype is not None else None
...
def get_exception_breakpoint(exctype, exceptions):
    exception_full_qname = (
        f'{str(exctype.__module__)}.{exctype.__name__}' if exctype else None
    )
    exc = None
    if exceptions is not None:
        try:
            ...
            for exception_breakpoint in exceptions.values():
                if (
                    exception_breakpoint.type is not None
                    and issubclass(exctype, exception_breakpoint.type)
                    and (
                        exc is None
                        or issubclass(exception_breakpoint.type, exc.type)
                    )
                ):
...
def stop_on_unhandled_exception(py_db, thread, additional_info, arg):
    ...
    if break_on_uncaught_exceptions := py_db.break_on_uncaught_exceptions:
    ...
    pydev_log.debug(
        f'Handling post-mortem stop on exception breakpoint {exception_breakpoint.qname}'
    )
