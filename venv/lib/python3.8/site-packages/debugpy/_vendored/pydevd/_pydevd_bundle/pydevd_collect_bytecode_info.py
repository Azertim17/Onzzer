...
class TryExceptInfo(object):
    ...
    def __str__(self):
        ...
        if self.raise_lines_in_except:
            lst.append(f" raises: {', '.join(str(x) for x in self.raise_lines_in_except)}")
...
def _get_line(op_offset_to_line, op_offset, firstlineno, search=False):
    ...
    raise AssertionError(
        f'Unable to find line for offset: {op_offset_original}.Info: {op_offset_to_line}'
    )
...
def iter_instructions(co):
    ...
    bytecode_to_instruction = {
        instruction.offset: instruction for instruction in iter_in
    }
    if iter_in:
        yield from iter_in
...
def collect_return_info(co, use_func_first_line=False):
    ...
    firstlineno = co.co_firstlineno if use_func_first_line else 0
    lst = []
    op_offset_to_line = dict(dis.findlinestarts(co))
...
if sys.version_info[:2] <= (3, 9):
    ...
    class _TargetInfo(object):
        ...
        def __str__(self):
            msg = ['_TargetInfo(', self.except_end_instruction.opname]
            if self.jump_if_not_exc_instruction:
                msg.extend(
                    (
                        ' - ',
                        self.jump_if_not_exc_instruction.opname,
                        '(',
                        str(self.jump_if_not_exc_instruction.argval),
                        ')',
                    )
                )
            msg.append(')')
    ...
    def collect_try_except_info(co, use_func_first_line=False):
        ...
        offset_to_instruction_idx = {
            instruction.offset: i for i, instruction in enumerate(instructions)
        }
        for i, instruction in enumerate(instructions):
            ...
            if curr_op_name in ('SETUP_FINALLY', 'SETUP_EXCEPT'):  # SETUP_EXCEPT before Python 3.8, SETUP_FINALLY Python 3.8 onwards.
                ...
                indexes_checked = {exception_end_instruction_index}
                ...
                if except_end_instruction is not None:
                    ...
                    for raise_instruction in instructions[i:offset_to_instruction_idx[try_except_info.except_end_bytecode_offset]]:
                        if (
                            raise_instruction.opname == 'RAISE_VARARGS'
                            and raise_instruction.argval == 0
                        ):
...
class _Disassembler(object):
    ...
    def _lookahead(self):
        ...
        if next_instruction.opname in ('CALL_FUNCTION', 'PRECALL'):
            ...
            args = list(found)
            first_instruction = args[0] if args else next_instruction
    ...
    def _next_instruction_to_str(self, line_to_contents):
        ...
        if instruction.opname == 'LOAD_CONST' and inspect.iscode(
            instruction.argval
        ):
        ...
        if instruction.opname == 'RAISE_VARARGS' and instruction.argval == 0:
