...
_BINARY_OPS = {opname for opname in dis.opname if opname.startswith('BINARY_')}
...
class Target(object):
    ...
    def __repr__(self):
        ret = [f'{s}: {getattr(self, s)}' for s in self.__slots__]
        return f"Target({', '.join(ret)})"
...
class _StackInterpreter(object):
    ...
    def _getcallname(self, instr):
        ...
        if not isinstance(name, str):
            return None
        return name.split('.')[-1] if name.endswith('>') else name
    ...
    def _handle_call_from_instr(self, func_name_instr, func_call_instr):
        ...
        if not call_name:
            ...
            if code_obj is not None:
                if children_targets := _get_smart_step_into_targets(code_obj):
    ...
    def on_MAKE_FUNCTION(self, instr):
        ...
        if call_name in (
            ...
            '<setcomp>',
            '<dictcomp>',
        ) and isinstance(code_obj_instr.arg, CodeType):
...
def _get_smart_step_into_targets(code):
    ...
    for block in cfg:
        ...
        for instr in block:
            try:
                func_name = f'on_{instr.name}'
                func = getattr(stack, func_name, None)

                if DEBUG and instr.name != 'CACHE':
        ...
        for s in self.__slots__:
            if s == 'parent':
                try:
                    ...
                    ret.append(f'{s}: <not set>')
                else:
                    if parent is None:
                        ret.append('parent: None')
                    else:
                        ret.append(f'parent: {parent.name} ({parent.offset})')
                continue

            if s == 'children_variants':
                ret.append(
                    f'children_variants: {len(self.children_variants) if self.children_variants else 0}'
                )
                continue

            try:
                ret.append(f'{s}: {getattr(self, s)}')
            except AttributeError:
                ret.append(f'{s}: <not set>')
        return f"Variant({', '.join(ret)})"
...
def _convert_target_to_variant(target, start_line, end_line, call_order_cache, lasti, base):
    ...
    if children_targets := target.children_targets:
        children_variants = [
...
def get_smart_step_into_variant_from_frame_offset(frame_f_lasti, variants):
    ...
    return None if i == 0 else variants[i - 1]
