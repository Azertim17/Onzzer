...
def main():
    ...
    try:
        ...
        if setup['multiprocess']:  # PyDev
            ...
            if f.find('dev_appserver.py') != -1 and os.path.basename(
                f
            ).startswith('dev_appserver.py'):
    ...
    if access_token := setup['access-token']:
        debugger.authentication.access_token = access_token

    if client_access_token := setup['client-access-token']:
    ...
    if fix_app_engine_debug:
        ...
        sys.argv.insert(1, f'--python_startup_script={app_engine_startup_file}')
        import json
        setup['pydevd'] = __file__
        sys.argv.insert(2, f'--python_startup_args={json.dumps(setup)}')
