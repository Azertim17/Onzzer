...
def process_template_file(contents):
    ...
    for line in contents.splitlines(keepends=False):
        if line.strip() == '### TEMPLATE_START':
            ...
            append_to = ret
            ret.extend(iter(process_template_lines(template_lines)))
        else:
...
def build_extension(dir_name, extension_name, target_pydevd_name, force_cython, extended=False, has_pxd=False, template=False):
    pyx_file = os.path.join(
        os.path.dirname(__file__), dir_name, f"{extension_name}.pyx"
    )
    ...
    if target_pydevd_name != extension_name:
        ...
        new_pyx_file = os.path.join(
            os.path.dirname(__file__), dir_name, f"{target_pydevd_name}.pyx"
        )
        new_c_file = os.path.join(
            os.path.dirname(__file__), dir_name, f"{target_pydevd_name}.c"
        )
        shutil.copy(pyx_file, new_pyx_file)
        pyx_file = new_pyx_file
        if has_pxd:
            pxd_file = os.path.join(
                os.path.dirname(__file__), dir_name, f"{extension_name}.pxd"
            )
            new_pxd_file = os.path.join(
                os.path.dirname(__file__),
                dir_name,
                f"{target_pydevd_name}.pxd",
            )
            shutil.copy(pxd_file, new_pxd_file)
        assert os.path.exists(pyx_file)

    try:
        c_files = [os.path.join(dir_name, f"{target_pydevd_name}.c")]
        if force_cython:
            ...
            target = f"{dir_name}/{target_pydevd_name}.pyx"
            cythonize([target])
...
''')
        ...
        ext_modules = [
            Extension(
                f'{dir_name}{"_ext" if extended else ""}.{target_pydevd_name}',
                c_files,
                **kwargs,
            )
        ]
