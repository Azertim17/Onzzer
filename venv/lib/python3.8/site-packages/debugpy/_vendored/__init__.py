...
def list_all(resolve=False):
    ...
    projects = ["pydevd"]
    return [project_root(name) for name in projects] if resolve else projects
...
def iter_project_files(project, relative=False, **kwargs):
    ...
    if relative:
        with _util.cwd(VENDORED_ROOT):
            yield from _util.iter_all_files(project, **kwargs)
    else:
        root = project_root(project)
        yield from _util.iter_all_files(root, **kwargs)
